...
const schema=z.object({id:z.string(),correct:z.boolean()});
...
export async function POST(req:Request){
  ...
  const item=await db.reviewQueueItem.findFirst({where:{id:input.id,studentId:user.student.id}});
  if(!item) return NextResponse.json({error:'Tekrar kaydı bulunamadı.'},{status:404});
  let step=item.stepIndex;
  if(input.correct) step=Math.min(step+1,REVIEW_DAYS.length);
  else step=0;
  const completed=input.correct && step>=REVIEW_DAYS.length;
  const due=new Date();
  if(!completed) due.setDate(due.getDate()+REVIEW_DAYS[step]);
  const row=await db.reviewQueueItem.update({where:{id:item.id},data:{
    stepIndex:step,lastCorrect:input.correct,status:completed?'COMPLETED':'PENDING',
