    ...
    {(data.screenings||[]).length===0?<div className="card muted">Yönetici onayı bekleyen eğilim taraması yok.</div>:(data.screenings||[]).map((a:any)=>{
      ...
      return <article className="card" key={a.id}>
        <div className="moduleHeaderRow">
          <div><div className="moduleEyebrow">YÖNETİCİ İNCELEMESİ</div><h2>{a.student.fullName}</h2><p className="muted">Kod: {a.student.studentCode} · {getAdultExamGroup(a.student.gradeLevel)?'Sınav grubu: '+displayExamGroupWithTrack(a.student.gradeLevel,a.student.academicTrack):(a.student.gradeLevel||'Düzey belirtilmedi')} · {new Date(a.completedAt).toLocaleString('tr-TR')} · Koç: {a.student.coach?.user?.name||'Atanmamış'}</p></div>
